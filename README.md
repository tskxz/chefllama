...
O ChefLLama é um agente culinário inteligente construído com LangChain 1.x, LangGraph, Ollama (Llama 3.2 e LLaVA) e pesquisa na web em tempo real através da API Tavily.
...
- Pesquisa Web em Tempo Real (Tavily): Encontra receitas reais, truques culinários e combinações na web.
- Modelos Locais com Ollama: Utiliza os modelos llama3.2 e llava localmente com execução rápida e privada.
- Memória de Conversação (LangGraph): Mantém o contexto de múltiplos turnos para fazer perguntas de seguimento, pedir o passo a passo de uma receita escolhida ou adaptar a restrições alimentares.
- Interface CLI Interativa: Permite conversar diretamente no terminal ou executar consultas diretas por argumento.
